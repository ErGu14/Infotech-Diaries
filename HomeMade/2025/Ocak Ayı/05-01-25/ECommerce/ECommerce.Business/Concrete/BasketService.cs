...
namespace ECommerce.Business.Concrete
{
    public class BasketService : IBasketService
    {
        ...
        public async Task<ResponseDTO<BasketItemDTO>> AddProductToBasketAsync(BasketItemCreateDTO basketItemCreateDTO)
        {
            ...
            //ürün daha önceden sepette varmı onu bulacağız
            var existingBasketItem = basket.BasketItems.FirstOrDefault(x=>x.ProductId==product.Id);
            // ürün daha önceden sepette var mı onu kontrol ediyoruz
        ...
        public async Task<ResponseDTO<NoContent>> ClearBasketAsync(string applicationUserId) // kullanıcı siparişi tamamladığı zaman sepetini temizlicez . burayı Order kısmında kullanacağız
        {
        ...
        public async Task<ResponseDTO<NoContent>> RemoveProductFromBasketAsync(int basketItemId) //sepette ilgili ürünü silme
        {
            var basketItem = await _unitOfWork.GetRepository<BasketItem>().GetByIdAsync(basketItemId); // sepetteki ürünü bul
            if(basketItem == null)
            {
