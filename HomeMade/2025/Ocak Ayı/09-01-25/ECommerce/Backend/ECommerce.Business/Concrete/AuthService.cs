...
namespace ECommerce.Business.Concrete
{
    public class AuthService : IAuthService
    {
        ...
        // kullanıcı giriş yaparken kullanacağı bir token oluşturma
        private async Task<TokenDTO> GenerateJwtToken(ApplicationUser user)
        {
            var roles = await _userManager.GetRolesAsync(user); // rolünü getir ve tokende rolüde göm

            var claims = new[] // gömülecek bilgileri array yani dizi olarak ekliyoruz
            {
                new Claim(ClaimTypes.NameIdentifier, user.Id),
                new Claim(ClaimTypes.Email, user.Email),
                new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString())
            }.Union(roles.Select(r => new Claim(ClaimTypes.Role, r))); // tek tek bildileri gir ve dolaraşark rol bilgisini ekle diyoruz yani rollerimiz birdne çok iste hepsini ekliyoruz
