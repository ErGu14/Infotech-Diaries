...
--Skills Tablosuna Örnek Ekleme
insert into Skills(Name,Rate)
values('HTML',95),('CSS',90),('JavaScript',85),('MVC',80)
GO
...
--trigger (tetikleyici yani þu olunca sen þunu yap yani örnek tabloda þu þu deðiþiklik olursa sen þunu yap)

create trigger trg_UpdateAt_Abouts --abouts tablosununa update edildiði zaman kodlarý yazýyoruz
on Abouts
after update
as
begin
	...
	from Abouts a
	inner join inserted i on a.Id=i.Id  -- o sýrada hangi kaydý güncellediðimizi tespit etmek için
end
go

create trigger trg_UpdateAt_Categories --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on Categories
after update
as
begin
	...
	set UpdatedAt= GETDATE()
	from Categories x
	inner join inserted i on x.Id=i.Id  -- o sýrada hangi kaydý güncellediðimizi tespit etmek için
end
go

create trigger trg_UpdateAt_Contacts --contacts tablosununa update edildiði zaman kodlarý yazýyoruz
on Contacts
after update
as
begin
...
go
create trigger trg_UpdateAt_HomeBanners --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on HomeBanners
after update
as
begin
...
go
create trigger trg_UpdateAt_Projects --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on Projects
after update
as
begin
...
create trigger trg_UpdateAt_ServiceInfos --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on ServiceInfos
after update
as
begin
...
go
create trigger trg_UpdateAt_Services --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on Services
after update
as
begin
...
go
create trigger trg_UpdateAt_Settings --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on Settings
after update
as
begin
...
go
create trigger trg_UpdateAt_SocialsMediaAccounts --categories tablosununa update edildiði zaman kodlarý yazýyoruz
on SocialsMediaAccount
after update
as
begin
	update SocialsMediaAccount
	set UpdatedAt= GETDATE()
	from SocialsMediaAccount x
	inner join inserted i on x.Id=i.Id  -- o sýrada hangi kaydý güncellediðimizi tespit etmek için
end
go